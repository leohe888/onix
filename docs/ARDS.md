...
地址范围描述符结构（Address Range Descriptor Structure，ARDS）是一种用于描述内存地址范围的结构，大小为 20 字节：
...
- 输入：
  ...
  - EDX：签名，固定为 `0x534d4150` (即 "SMAP" 的 ASCII 码)。
  - ES:DI：ARDS 缓存地址。

- 输出：
  - CF 位为 0 表示成功，为 1 表示失败。
  - EAX：`0x534d4150` (即 "SMAP" 的 ASCII 码)。
  - EBX：续传值。为 0 表示这是最后一个 ARDS，没有更多 ARDS 了。
  - ECX：BIOS 返回的 ARDS 大小，以字节为单位，一般为 `20`。

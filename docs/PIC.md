...
PIC（Programmable Interrupt Controller，可编程中断控制器）是微机系统中用于管理中断的硬件，它能够接收多个中断源的中断请求，并按照一定的优先级顺序向 CPU 发送中断信号，从而实现中断管理。

Intel 8259A 是最经典的 PIC 芯片，其结构如下：
...
单个 8259A 可管理 8 路中断，多个 8259A 级联最多可扩展至 64 路。一般使用 2 个 8259A 进行级联，一个主 8259A，一个从 8259A，从 8259A 的 INT 连接到主 8259A 的 IRQ2。
...
- IRQ1：键盘控制器。
- IRQ2：级联从 8259A。
- IRQ3：串行端口 2/4（COM2/COM4）。
- IRQ4：串行端口 1/3（COM1/COM3）。
- IRQ5：并行端口 2（LPT2）/ 声卡。
- IRQ6：软盘控制器。
- IRQ7：并行端口 1（LPT1）。
